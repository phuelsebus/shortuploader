...
async function runUploads(jobId: string, platforms: Platform[], job: UploadJob): Promise<void> {
  await Promise.allSettled(
    platforms.map(async (platform) => {
      ...
      try {
        ...
        const e = err as { platform?: Platform; code?: string; message?: string; retryable?: boolean };
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const tags: string[] =
      typeof req.body.tags === 'string' ? (JSON.parse(req.body.tags) as string[]) : [];
    ...
    if (!title || platforms.length === 0) {
      fs.unlinkSync(file.path);
      res.status(400).json({ success: false, error: 'title and at least one platform are required' });
      return;
    }
