...
import { getInstagramAuthUrl, exchangeInstagramCode } from '../services/instagram';
...
router.get('/:platform/callback', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!code || state !== req.session.oauthState || platform !== req.session.oauthPlatform) {
  ...
  try {
    ...
    res.redirect(`${CLIENT_ORIGIN()}/auth/error?reason=exchange_failed&platform=${platform}`);
  }
});
