  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {platforms.map((platform) => (
        ...
        >
          <span className="text-white/40">{PLATFORM_ICONS[platform]}</span>
    ...
    <span className="flex items-center gap-2 text-xs text-red-400">
      ...
      {status.error?.retryable && onRetry && (
        <button
          ...
          className="rounded-md border border-red-500/30 bg-red-500/10 px-2 py-0.5 text-xs font-medium text-red-400 hover:bg-red-500/20 transition-colors"
        >
          Retry
        </button>
      )}
